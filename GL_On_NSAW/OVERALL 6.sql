...
SELECT
    ...
    a.GLAccountStatus,
    a.GLAccountCategory,
    TO_CHAR(j.GJEntryDate, 'MMDDYYYY') AS GLDate,
    CASE
        WHEN g.GJAllocationAmount > 0
            THEN CAST(ABS(TO_NUMBER(g.GJAllocationAmount)) AS NUMBER)
...
FROM
    PCLAW_GJEntry j
JOIN PCLAW_GJAlloc g
    ON j.GJEntryID = g.GJAllocationGJID
JOIN PCLAW_GLAcct a
    ON g.GLAcctID = a.GLAcctID
JOIN PCLAW_TranIDX x
    ON j.GJEntryID = x.TranIndexSequenceID
WHERE
    x.TranIndexStatus = 0
Union all
SELECT
    CAST('2' AS VARCHAR2(10)) AS Sno,
    CAST('GBComm' AS VARCHAR2(20)) AS TableName,
    CAST('GB' AS VARCHAR2(10)) AS Journal,
    a.GLAcctID,
    a.GLAccountAcctName,
    a.GLAccountNickName,
    a.GLAccountStatus,
    a.GLAccountCategory,
    TO_CHAR(Comm.GBankCommInfDate, 'YYYYMMDD') AS GLDate,
    CASE
        WHEN Comm.GBankCommInfEntryType IN (1300, 1301, 1303)
             AND Comm.GBankCommInfAmount >= 0
            THEN CAST(TO_NUMBER(Comm.GBankCommInfAmount) AS NUMBER)
        WHEN Comm.GBankCommInfEntryType NOT IN (1300, 1301, 1303)
             AND Comm.GBankCommInfAmount < 0
            THEN CAST(ABS(TO_NUMBER(Comm.GBankCommInfAmount)) AS NUMBER)
        ELSE CAST(0 AS NUMBER)
    END AS DebitAmount,
    CASE
        WHEN Comm.GBankCommInfEntryType NOT IN (1300, 1301, 1303)
        ...
        ELSE CAST(0 AS NUMBER)
    END AS CreditAmount,
    CAST('Bank Entry' AS VARCHAR2(4000)) AS Explanation,
...
JOIN
    PCLAW_GBAcctI I
    ON Comm.GBankCommInfAccountID = I.GBankAcctInfBankAccountID
JOIN
    PCLAW_GLAcct a
    ON I.GBankAcctInfGLAccountID = a.GLAcctID
WHERE
    Comm.GBankCommInfStatus = 0
    AND Comm.GBankCommInfEntryType BETWEEN 1201 AND 1499
union all
SELECT
    ...
    CASE
        WHEN GBankAllocInfEntryType IN (1400, 2001, 2000)
            ...
            THEN CAST(TO_NUMBER(GBankAllocInfAmount) AS NUMBER)
        WHEN GBankAllocInfEntryType IN (1100, 1101, 1102, 1103, 1104, 1300, 1301)
             AND GBankAllocInfAmount < 0
            THEN CAST(ABS(TO_NUMBER(GBankAllocInfAmount)) AS NUMBER)
        ELSE CAST(0 AS NUMBER)
    END AS DebitAmount,
    CASE
        WHEN GBankAllocInfEntryType IN (1100, 1101, 1102, 1103, 1104, 1300, 1301)
...
FROM
    PCLAW_GBAlloc Alloc
JOIN PCLAW_GLAcct a
    ON Alloc.GBankAllocInfGLID = a.GLAcctID
...
LEFT JOIN
    PCLAW_TranIDX t
    ON Alloc.GBankAllocInfAllocID = t.TranIndexSequenceID
WHERE
    t.TranIndexStatus = 0
    AND GBankAllocInfEntryType IN (1400, 1600, 1899)
...
SELECT
    ...
    CASE
        WHEN SUM(TO_NUMBER(Alloc.GBankAllocInfAmount)) >= 0
            THEN CAST(SUM(TO_NUMBER(Alloc.GBankAllocInfAmount)) AS NUMBER)
        ELSE CAST(0 AS NUMBER)
    END AS DebitAmount,
    CASE
        WHEN SUM(TO_NUMBER(Alloc.GBankAllocInfAmount)) < 0
            THEN CAST(ABS(SUM(TO_NUMBER(Alloc.GBankAllocInfAmount))) AS NUMBER)
        ELSE CAST(0 AS NUMBER)
    END AS CreditAmount,
...
WHERE
    t.TranIndexStatus = 0
    AND Alloc.GBankAllocInfEntryType IN (1400, 1600, 1899)
    AND Alloc.MatterID <> 0
GROUP BY
...
SELECT
    ...
    CASE
        WHEN TBankCommInfEntryType = 2050
             AND TBankAllocInfoAmount > 0
            THEN CAST(TO_NUMBER(TBankAllocInfoAmount) AS NUMBER)
        WHEN TBankCommInfEntryType = 2050
             AND TBankAllocInfoAmount < 0
            THEN CAST(0 AS NUMBER)
        WHEN TBankCommInfEntryType IN (2054)
             OR (
...
JOIN
    PCLAW_TBAcctI I
    ON Comm.TBankCommInfAccountID = I.TBankAcctInfBankAccountID
JOIN
    PCLAW_GLAcct a
    ON I.TBankAcctInfGLAccountID = a.GLAcctID
JOIN
    PCLAW_TBAlloc Alloc
...
SELECT
    ...
    SUM(
        CASE
            WHEN TBankAllocInfoEntryType IN (2050, 2054)
                THEN CAST(0 AS NUMBER)
...
JOIN
    PCLAW_GLAcct b
        ON I.TBankAcctInfGLAccountID = b.GLAcctID
WHERE
...
SELECT
    ...
    CASE
        WHEN Alloc.GBankAllocInfAmount < 0
            THEN CAST(ABS(TO_NUMBER(Alloc.GBankAllocInfAmount)) AS NUMBER)
        ELSE CAST(0 AS NUMBER)
    END AS DebitAmount,
    CASE
        WHEN Alloc.GBankAllocInfAmount >= 0
            THEN CAST(TO_NUMBER(Alloc.GBankAllocInfAmount) AS NUMBER)
        ELSE CAST(0 AS NUMBER)
    END AS CreditAmount,
...
JOIN
    PCLAW_GBAlloc Alloc
    ON I.APInvoiceID = Alloc.GBankAllocInfCheckID
JOIN
    PCLAW_GLAcct a
    ON Alloc.GBankAllocInfGLID = a.GLAcctID
LEFT JOIN
    PCLAW_ActCode C
    ON Alloc.GBankAllocInfActivityID = C.ActivityCodesID
LEFT JOIN
    PCLAW_MattInf m
    ON Alloc.MatterID = m.MatterID
LEFT JOIN
    PCLAW_APVendLi v
    ON I.APInvoiceVendorID = v.APVendorListID
WHERE
    APInvoiceStatus = 0
    AND m.MatterID <> 0
union all
...
Where T.GLDate between 20000101 and 20001231