SELECT
    ...
    /* DEBIT AMOUNT */
    CASE
        WHEN ALLOC.GJAllocationAmount > 0 THEN ALLOC.GJAllocationAmount
        ELSE 0
    END AS DEBITAMOUNT,
    /* CREDIT AMOUNT */
    CASE
    ...
    ENTRY.GJEntryRef as ReferenceNumber,
    ENTRY.GJEntryID as EntryNumber
FROM
    PCLAW_GJENTRY ENTRY
    JOIN PCLAW_GJALLOC ALLOC ON ENTRY.GJENTRYID = ALLOC.GJALLOCATIONGJID
    JOIN PCLAW_GLACCT ACCT ON ALLOC.GLAcctID = ACCT.GLACCTID
union
all
select
    'PP' AS JOURNAL,
    ...
    /* Debit amount */
    CASE
        WHEN Comm.GBankCommInfEntryType IN (1300, 1301, 1303) THEN Comm.GBankCommInfAmount
        ELSE 0
    END AS DEBITAMOUNT,
    /* Credit amount */
    CASE
        WHEN COMM.GBankCommInfEntryType IN (1300, 1301, 1303) THEN 0
...
from
    PCLAW_GBComm COMM
    join PCLAW_GBAcctI ACCT_1 on Comm.GBankCommInfAccountID = ACCT_1.GBankAcctInfBankAccountID
    join PCLAW_GLAcct ACCT on ACCT_1.GBankAcctInfGLAccountID = ACCT.GLAcctID
where
    COMM.GBankCommInfStatus = 0
    and COMM.GBankCommInfEntryType > 1200
    and COMM.GBankCommInfEntryType < 1500
union
all
SELECT
    CASE
    ...
    /* Debit amount */
    CASE
        WHEN ALLOC.GBankAllocInfEntryType IN (1400, 2000, 2001) THEN ALLOC.GBankAllocInfAmount
        ELSE 0
    END AS DEBITAMOUNT,
    /* Credit amount */
    CASE
        WHEN ALLOC.GBankAllocInfEntryType IN (1100, 1101, 1102, 1103, 1104, 1300, 1301) THEN ALLOC.GBankAllocInfAmount
        ELSE 0
    END AS CREDITAMOUNT,
...
FROM
    PCLAW_GBCOMM COMM
    JOIN PCLAW_GBALLOC ALLOC ON ALLOC.GBankAllocInfCheckID = COMM.GBankCommInfID
    JOIN PCLAW_GLACCT ACCT ON ALLOC.GBankAllocInfGLID = ACCT.GLAcctID
WHERE
    COMM.GBankCommInfStatus = 0
    AND ALLOC.GBankAllocInfEntryType NOT IN (1600, 1650, 1651, 1652, 6500, 1803, 1899, 1200)
    AND COMM.GBankCommInfEntryType NOT IN (1900, 1901, 1902, 1903, 1904, 1905)
...
FROM
    ...
    LEFT JOIN PCLAW_TranIDX TRAN ON ALLOC.GBankAllocInfAllocID = TRAN.TranIndexSequenceID
WHERE
    TRAN.TranIndexStatus = 0
...
FROM
    PCLAW_TBAlloc ALLOC
    JOIN PCLAW_GLAcct ACCT ON ACCT.GLAccountNickName = '23010'
...
FROM
    PCLAW_APInv INV
    JOIN PCLAW_GBAlloc ALLOC ON INV.APInvoiceID = ALLOC.GBankAllocInfCheckID
    JOIN PCLAW_GLAcct ACT ON ALLOC.GBankAllocInfGLID = ACT.GLAcctID
    LEFT JOIN PCLAW_ActCode ACTC ON ALLOC.GBankAllocInfActivityID = ACTC.ActivityCodesID
    LEFT JOIN PCLAW_MattInf MAT ON ALLOC.MatterID = MAT.MatterID
    LEFT JOIN PCLAW_TBComm COMM ON ALLOC.GBankAllocInfCheckID = COMM.TBankCommInfSequenceID
WHERE
...
FROM
    PCLAW_APInv INV
    JOIN PCLAW_GBAlloc ALLOC ON INV.APInvoiceID = ALLOC.GBankAllocInfCheckID
    JOIN PCLAW_GLAcct ACCT ON ACCT.GLAccountNickName = '22010'
...
from
    pclaw_arinv inv
    join pclaw_arlwyspl spl on inv.invoiceid = spl.invoiceid
    join pclaw_glacct acct on spl.arlawyersplitlawyerid = acct.glaccountforlawyer
    and acct.glaccountspecacct = 13
    and acct.glaccountstatus = 0
    join pclaw_lawinf law on spl.arlawyersplitlawyerid = law.lawyerid
where
    inv.arinvoicestatus = 0
...
from
    pclaw_arinv inv
    join pclaw_arlwyspl law on inv.invoiceid = law.invoiceid
    join pclaw_glacct acct on acct.glaccountnickname = '12020'
    join pclaw_lawinf inf on law.arlawyersplitlawyerid = inf.lawyerid
where
    inv.arinvoicestatus = 0
...
from
    pclaw_arinv inv
    join pclaw_arlwyspl spl on inv.invoiceid = spl.invoiceid
    join pclaw_tranidx idx on spl.woid = idx.tranindexsequenceid
    join pclaw_glacct acct on spl.arlawyersplitglid = acct.glacctid
    left join pclaw_lawinf inf on spl.arlawyersplitlawyerid = inf.lawyerid
    join pclaw_arwo wo on spl.woid = wo.woid
where
    idx.tranindexstatus = 0
union
all
select
    'wo' as journal,
...
from
    pclaw_arinv inv
    join pclaw_arlwyspl law on inv.invoiceid = law.invoiceid
    join pclaw_tranidx t on law.woid = t.tranindexsequenceid
    join pclaw_glacct acct on acct.glaccountnickname = '12020'
    left join pclaw_lawinf inf on law.arlawyersplitlawyerid = inf.lawyerid
    join pclaw_arwo wo on law.woid = wo.woid
where
    t.tranindexstatus = 0
union
all
...
FROM
    PCLAW_ARInv inv
    JOIN PCLAW_GLAcct acct ON acct.GLAccountNickName = '22060'
WHERE
    inv.ARInvoiceStatus = 0
    AND inv.ARInvoiceGSTFees <> 0
union
...
FROM
    PCLAW_ARInv inv
    JOIN PCLAW_GBRcptA r ON inv.InvoiceID = r.GBankARRcptAllocInvID
    JOIN PCLAW_GLAcct acct ON acct.GLAccountNickName = '12020'
WHERE
    inv.ARInvoiceStatus = 0
...
FROM
    PCLAW_GBAlloc alloc
    JOIN PCLAW_GLAcct acct ON acct.GLAccountNickName = '22070'
    JOIN PCLAW_GBComm comm ON alloc.GBankAllocInfCheckID = comm.GBankCommInfID
WHERE
    alloc.GBankAllocInfStatus = 0
