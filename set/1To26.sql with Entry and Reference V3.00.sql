...
FROM
	(
		...
		from
			...
			join [PCLAWDB_32130].[dbo].GLAcct a on I.TBankAcctInfGLAccountID = a.GLAcctID --where TBankCommInfStatus = 0 and TBankCommInfEntryType not in (1552, 1553, 2501) -- not sure why this item is excluded from the reports
			--7
		union
		...
		select
			...
			'' as GBankCommInfPaidTo,
			'' as ReferenceNumber,
			'' as EntryNumber --select SUM(GBankAllocInfAmount)
		from
			[PCLAWDB_32130].[dbo].APInv I
			join [PCLAWDB_32130].[dbo].GBAlloc Alloc on I.APInvoiceID = Alloc.GBankAllocInfCheckID
...
WHERE
	...
	AND 20240131
	AND T.ReferenceNumber IN ('23133094', '23132901', '23132048', '12/05/23') --ORDER BY T.GLDate ASC
	--GROUP BY T.TableName, T.Journal, T.GLAcctID, T.GLAccountAcctName, T.GLAccountNickName, T.GLAccountStatus, T.GLAccountCategory