...
SELECT *
--T.GLAccountNickName,sum(DebitAmount),sum(CreditAmount),count(*)
FROM
    (
        select
            '1' as Sno,
            ...
            TranIndexEntryType as EntryType,
            case
        ...
        select
            ...
            GBankCommInfEntryType as EntryType,
            CASE
        ...
        select
            ...
            a.GLAccountCategory,
            (GBankCommInfDate) as GLDate,
            GBankAllocInfEntryType as EntryType,
        ...
        select
            ...
            MAX(GBankCommInfEntryType) as EntryType,
            ...
            '' as ReferenceNumber,
            '' as EntryNumber,
            SUM(GBankAllocInfAmount) as amount
        from
            [PCLAWDB_32130].[dbo].GBAlloc Alloc
        ...
        select
            ...
            TBankCommInfEntryType as EntryType,
            CASE
    WHEN TBankCommInfEntryType = 2050 AND TBankAllocInfoAmount > 0 THEN CONVERT(money, TBankAllocInfoAmount)
    WHEN TBankCommInfEntryType = 2050 AND TBankAllocInfoAmount < 0 THEN CONVERT(money, 0.00)
...
CASE
    WHEN TBankCommInfEntryType = 2050 AND TBankAllocInfoAmount < 0 THEN CONVERT(money, ABS(TBankAllocInfoAmount))
    WHEN TBankCommInfEntryType = 2050 AND TBankAllocInfoAmount > 0 THEN CONVERT(money, 0.00)
...
END AS CreditAmount,
            ...
            TBankAllocInfoAmount as amount
        from
		...
		'7' as Sno,
            ...
            MAX(TBankCommInfEntryType) as EntryType,
			SUM(Case
			...
			'' as ReferenceNumber,
			'' as EntryNumber,
            SUM(TBankAllocInfoAmount) as amount
		from
    ...
    --AND
    --T.GLAccountNickName = '1020'
    --And
    --comment like '%Total%'
    --and
	T.GLDate BETWEEN 20100101 AND 20101231
    --and T.EntryNumber IN ('164785','196171','196607','201483','205313','201645','202059','207264','213308','215952')
	--and T.Journal != 'GB'
